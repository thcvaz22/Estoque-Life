...
const SYSTEM_VERSION = '16.2.0-login-estavel-aion-1.1';
...
router.get('/health', (req, res) => {
  ...
  res.json({ ok: true, time: new Date().toISOString(), systemVersion: SYSTEM_VERSION, cloudMode, storage: cloudMode ? 'sqlite-persistent-disk' : 'sqlite-local', publicBaseUrl, sellerUrl: publicBaseUrl ? publicBaseUrl.replace(/\/$/,'') + '/vendas/' : null, lanUrls: cloudMode ? [] : getLanUrls(httpsPort, 'https') });
});
